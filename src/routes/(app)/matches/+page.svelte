<script lang="ts">
	...
	const nonPersonalUserTeams = $derived(
		(data as any).userTeams?.filter((t: any) => !t.isPersonal) ?? []
	);
		...
		<div class="flex gap-2">
			{#if (data as any).userTeams?.length > 0}
				<button
					...
					class="font-600 rounded-md border border-accent/30 px-4 py-2 text-sm text-accent transition-colors hover:bg-accent/10"
				>
