	...
	{#if user}
		<div class="relative" data-profile-menu>
			...
			>
				...
				<svg class="h-3.5 w-3.5 text-text-secondary transition-transform {menuOpen ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
			...
			{#if menuOpen}
				<div
					class="absolute right-0 top-full z-50 mt-1.5 w-44 rounded-md border border-border bg-surface-800 py-1 shadow-lg"
				>
					<a
						href="/settings"
						onclick={closeMenu}
