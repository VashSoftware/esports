...
export const actions = {
	...
	rename: async ({ request, locals }: import('./$types').RequestEvent) => {
		const form = await request.formData();
	...
	addMember: async ({ request, locals }: import('./$types').RequestEvent) => {
		requireAuth(locals);
	...
	removeMember: async ({ request, locals }: import('./$types').RequestEvent) => {
		requireAuth(locals);
	...
	deleteTeam: async ({ request, locals }: import('./$types').RequestEvent) => {
		const form = await request.formData();
		const teamId = form.get('teamId')?.toString();
